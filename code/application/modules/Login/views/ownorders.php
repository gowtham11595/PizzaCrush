...
<a href="<?php echo base_url();?>owndelivered/<?php echo $value['id']; ?>"><button>Delivered</button></a>
<h4><?php print_r('OrderedBy: '.$user); ?></h4>
<?php
$name=$this->db->select('name')->from('breads')->where('id',$value['iid'])->get()->result();
$name=$name[0];
$name=(array)$name;
$name=$name['name'];
print_r('Bread:'.$name);$id=$value['id'];
	}
   if($value['type']=='toppings')
   {
	   $name=$this->db->select('name')->from('toppings')->where('id',$value['iid'])->get()->result();
       $name=$name[0];
       $name=(array)$name;
       $name=$name['name'];
       print_r('Topping:'.$name);

   }	   
      if($value['type']=='sauce')
   {
	   $name=$this->db->select('name')->from('sauce')->where('id',$value['iid'])->get()->result();
       $name=$name[0];
       $name=(array)$name;
       $name=$name['name'];
       print_r('Sauce:'.$name);
